...
- [ ] Every nav item (CONTOUR · SHOP · SHOP BY FIT · FIT FINDER · OUR APPROACH · SIZE GUIDE · ABOUT) resolves to a live route; no orphans.
- [ ] `/shop-by-fit/[category]` and `/products/[id]` handle unknown params with a quiet not-found.
- [ ] One type scale, one palette, one spacing rhythm verified across all routes: `/`, `/shop`, `/shop-by-fit`, `/shop-by-fit/[category]`, `/fit-finder`, `/our-approach`, `/size-guide`, `/about`, `/products/[id]`.
- [ ] Full `npm run evidence` sweep green: Lighthouse ≥ 95 ×4 per route, axe clean, colour-audit pass, zero console errors, CLS < 0.05.
